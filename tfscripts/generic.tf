...
;; create air hammers and throw away the old ones. Only keeps ones with >= 26 dr
...
/def -ahCyellow -F -mregexp -t"^'air hammer     '  modifies damage roll by  (2[6-9]|3[0-9]) continuous" air_hammer02 = /if /test $(/getvar auto_hammer) == 1%;/then put ham %{main_bag}%;/endif
