...
/def -F -mregexp -t"^(.*) shimmers with a gold aura\.$" auto_enchant_armor2=/if /test $(/getvar auto_enchant) == 1%;/then /send put "%{ae_keywords}" %{fodder_bag}=c id "%{ae_keywords}"%;/endif
...
/def -F -mregexp -t"^.* flares blindingly\.\.\. and evaporates\!" auto_enchant_armor3=/if /test $(/getvar auto_enchant) == 1%;/then c id "%{ae_keywords}"%;/endif
