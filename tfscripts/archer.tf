...
/def -mregexp -t"You don\'t have enough mana to make ([a-zA-Z]+) ([a-zA-Z ]+)\." archer_nofletchmana = \
    ...
    /if ({autofletch} == 1) \
        /def full_mana_action = wa;fletch %{_projectileType} %{_arrowType}%;\
        /send sleep%;\
    /endif
...
/def -i -mglob -t"You discard your empty toolkit." archer_nofletchkit = \
    /if ({autofletch} = 1 & {tofletch} > 0) \
        /send get fletch %{main_bag}=wear fletch=fletch %fletchtype %arrowtype%; \
    /endif

/alias mkarrow \
    /if ({#} != 3) \
        /echo -p %%% @{hCred}Syntax: @{nCwhite}mkarrow @{xCyellow}<num> <arrow|bolt|stone> <type>%; \
    /else \
